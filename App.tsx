...
import { generateIdeas, generateOutline, generateScript, generateImage, reviewAndStructureScript, generatePublishingKit, generateKeywordAnalysis, generateMusicPrompts } from './services/geminiService';
...
const LOCAL_STORAGE_KEY = 'youtubeProductionAssistantState_v4'; // Incremented version
...
const App: React.FC = () => {
    ...
    const [vbeeAudioStates, setVbeeAudioStates] = useState<Record<number, VBeeAudioState>>({});
    ...
    }, [currentStep, topic, platform, videoType, ideas, selectedIdea, scriptOutline, keywordAnalysis, script, structuredScript, musicPrompts, publishingKit, thumbnailImageState, imageStyle, images, selectedVbeeVoice, vbeeAudioStates]);
...
${publishingKit.thumbnailConcepts.map((c, i) => `**Ý tưởng ${i+1}:** ${c.concept}\n**Prompt:** ${c.prompt}`).join('\n\n')}
    ...
    const handleGenerateVbeeAudio = useCallback(async (sceneNumber: number, text: string) => {
        ...
        try {
            // Note: In a real deployment, '/api/vbee' needs to point to your backend/serverless function.
            const response = await fetch('/api/vbee', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ text, voice: selectedVbeeVoice })
            });
            ...
            setVbeeAudioStates(prev => ({ ...prev, [sceneNumber]: { isLoading: false, audioUrl: data.audioUrl } }));
        } catch (err) {
            const message = err instanceof Error ? err.message : "Không thể tạo âm thanh.";
    ...
    const renderContent = () => {
        ...
        switch (currentStep) {
                 ...
                 return (
                    <div>
                        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                            <h2 className="text-2xl font-bold text-center sm:text-left text-slate-100">Tạo hình ảnh cho video</h2>
            ...
            case Step.Voiceover: {
                return (
                    <div>
                        <h2 className="text-2xl font-bold text-center mb-4 text-slate-100">Lồng tiếng chất lượng cao với VBee</h2>
                        <p className="text-center text-slate-400 mb-8 max-w-3xl mx-auto">Chọn một giọng nói chuyên nghiệp, sau đó tạo âm thanh cho từng cảnh. Bạn có thể tải về các tệp MP3 để sử dụng trong phần mềm dựng phim của mình.</p>
                        ...
                        <div className="space-y-6 max-w-3xl mx-auto">
                            {structuredScript.map((scene) => {
                                const audioState = vbeeAudioStates[scene.scene];
                                return (
                                <Card key={scene.scene}>
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center gap-3">
                                        ...
                                        <button onClick={() => handleGenerateVbeeAudio(scene.scene, scene.dialogue)} disabled={audioState?.isLoading} className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-500 rounded-md font-semibold text-sm text-white transition-colors disabled:bg-slate-700 disabled:cursor-wait">
                                            {audioState?.isLoading ? <><LoadingSpinner size="sm" /><span>Đang tạo...</span></> : <><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M7 4a3 3 0 016 0v6a3 3 0 11-6 0V4z" /><path d="M5.5 8.5a.5.5 0 01.5.5v1a3.5 3.5 0 007 0v-1a.5.5 0 011 0v1a4.5 4.5 0 01-4.5 4.472V16h1a.5.5 0 010 1h-3a.5.5 0 010-1h1v-1.028A4.5 4.5 0 014 10v-1a.5.5 0 01.5-.5z" /></svg><span>Tạo âm thanh</span></>}
                                        </button>
                                    </div>

                                    {audioState?.error && <p className="text-red-400 text-xs mt-3 text-center sm:text-left">{audioState.error}</p>}
                                    {audioState?.audioUrl && (
                                        <div className="mt-4 p-3 bg-slate-900/50 rounded-lg flex flex-col sm:flex-row items-center gap-4">
                                            <audio controls src={audioState.audioUrl} className="w-full sm:flex-grow">Trình duyệt không hỗ trợ.</audio>
                                            <a href={audioState.audioUrl} download={`scene_${scene.scene}_voiceover.mp3`} className="w-full sm:w-auto flex-shrink-0 flex items-center justify-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-500 rounded-md font-semibold text-sm text-white transition-colors">
